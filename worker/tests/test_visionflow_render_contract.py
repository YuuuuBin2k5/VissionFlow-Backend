...
class VisionFlowRenderContractTests(unittest.TestCase):
    def test_builds_mysql_free_short_form_contract(self):
        ...
        self.assertEqual(64, len(contract.render_plan_hash))
        self.assertFalse(hasattr(contract, "composition"))

    def test_rejects_non_vertical_format(self):
        with self.assertRaisesRegex(ValueError, "9:16"):
            build_visionflow_render_contract(
                "run-1", "a" * 32, {"input_payload": {"aspect_ratio": "16:9"}}, "script", [{"visual_search_keywords": "rain"}], {"state": "locked", "version_id": "composition-version-1", "aspect_ratio": "16:9", "tracks": []},
            )
