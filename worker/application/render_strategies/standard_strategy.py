...
class StandardRenderStrategy(RenderStrategy):
    ...
    async def _run_llm(self, job: dict, metadata: dict, contract: RenderContract, repo: VideoJobRepository):
        ...
        # NarrationHandoffCoordinator will dynamically fetch context from the Control Plane API
        # based on the legacy job_id. No need to pre-fetch using from_env() here.
        handoff_coordinator = NarrationHandoffCoordinator()
        handoff_coordinator.handle_narration(
            job_id, hook, full_script, scenes_payload_for_db, seo_tags, context=None
        )
        log_realtime_progress(job_id, "LLM_SCRIPT", "INFO",
