...
// Hàm tự động ưu tiên và lọc riêng kết nối tài khoản YouTube "@AsinMochiiBonii" cho bot này
...
// Lấy danh sách kênh YouTube thuộc về bot này (AsinMochiiBonii)
// Chỉ hiển thị kênh AsinMochii, loại trừ kênh Góc Chiêm Nghiệm để tránh hiển thị nhầm
...
const token = process.env.YOUTUBE_SECOND_TELEGRAM_BOT_TOKEN;
if (!token || token === 'YOUR_YOUTUBE_SECOND_TELEGRAM_BOT_TOKEN_HERE') {
    console.warn('[YouTube Second Bot] WARNING: YOUTUBE_SECOND_TELEGRAM_BOT_TOKEN is not configured. Bot will not start.');
}
...
const youtubeBot = new Telegraf(token || '', { handlerTimeout: telegramHandlerTimeoutMs });
export { youtubeBot as youtubeSecondBot };

youtubeBot.telegram.setMyCommands(YOUTUBE_BOT_COMMANDS).catch((error) => {
  console.warn('[YouTube Second Bot Warning] Could not register command menu:', error?.message || error);
});

youtubeBot.catch((err: any, ctx) => {
  if (isTelegrafTimeout(err)) {
    console.warn(
      `[YouTube Second Bot Warning] Handler xử lý quá ${telegramHandlerTimeoutMs}ms cho update ${ctx.updateType}. ` +
      'Nếu thao tác này thường xuyên chậm, hãy chuyển phần xử lý dài sang hàng đợi nền.'
    );
    return;
  }
  console.error(`[YouTube Second Bot Error] Error for ${ctx.updateType}:`, err);
  try {
    ctx.reply(`🚨 THÔNG BÁO LỖI HỆ THỐNG YOUTUBE SECOND BOT: ${err.message || err}`);
  } catch (replyErr) {
    console.error('[YouTube Second Bot Error] Failed to send error reply:', replyErr);
  }
});
...
async function startYouTubeTranslateDubSession(ctx: any) {
  try {
    ...
    console.error('[YouTube Second Bot Error] Error starting translate dub session:', error);
...
youtubeBot.command('__removed_startcampaign', async (ctx) => {
  try {
    ...
    if (paramText.includes('|')) {
      ...
      topic = paramText;
      const isPhilosophy = 
        topic.startsWith('[triết lý]') || 
        topic.startsWith('[philosophy]') || 
        topic.toLowerCase().includes('triết lý') || 
        topic.includes('"') || 
        topic.includes('“') ||
        topic.includes('”') ||
        topic.length > 45;
      
      targetAudience = isPhilosophy 
        ? 'Người trưởng thành chiêm nghiệm cuộc sống' 
        : 'Khán giả xem YouTube quan tâm chủ đề này';
    }
    ...
    return ctx.replyWithHTML(
      `🚀 <b>KHỞI CHẠY CHIẾN DỊCH YOUTUBE TỪ LỆNH GÕ</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━━\n` +
      `▪️ Chủ đề: <b>${escapeHtml(topic)}</b>\n` +
      `▪️ Đối tượng: <b>${escapeHtml(targetAudience)}</b>\n\n` +
      ...
      Markup.inlineKeyboard([
        [
          Markup.button.callback('🚀 Hôm Nay', 'yt:camp_wizard:launch_today'),
          Markup.button.callback('📅 Ngày Mai', 'yt:camp_wizard:launch_tomorrow')
        ],
        [Markup.button.callback('❌ Hủy bỏ', 'yt:camp_wizard:cancel')]
      ])
    );
  } catch (error: any) {
    console.error('Error starting YouTube campaign from command:', error);
    ctx.reply(`❌ Lỗi khi khởi chạy chiến dịch: ${error.message}`);
  }
});


youtubeBot.command('__removed_cancel_campaign', async (ctx) => {
  try {
    ...
    const confirmation = createConfirmation(
      ctx.chat.id,
      'cancel_campaign',
      `Ngừng kích hoạt chiến dịch Campaign #${campaignId} trên YouTube.`,
      { campaignId }
    );

    await ctx.reply(
      `❓ XÁC NHẬN YÊU CẦU HỦY CHIẾN DỊCH YOUTUBE:\n━━━━━━━━━━━━━━━━━━━━━\n${confirmation.summary}`,
      Markup.inlineKeyboard([
        Markup.button.callback('✅ Xác nhận', `yt:confirm:${confirmation.id}`),
        Markup.button.callback('❌ Hủy bỏ', `yt:deny:${confirmation.id}`),
      ])
